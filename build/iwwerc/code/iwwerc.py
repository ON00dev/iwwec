...
# Função para encontrar todas as variáveis, funções, classes e métodos no código
def find_identifiers(code):
    identifiers = set(re.findall(r'\b[a-zA-Z_][a-zA-Z0-9_]*\b', code))
    keywords = {'int', 'float', 'return', 'void', 'if', 'else', 'for', 'while', 'do', 'switch', 'case', 'break',
                'continue', 'typedef', 'struct', 'union', 'enum', 'const', 'volatile', 'static', 'extern', 'auto',
                'register', 'goto', 'sizeof', 'signed', 'unsigned', 'long', 'short', 'double', 'char', 'bool', 'inline',
                'restrict', 'default', 'sizeof', 'class', 'public', 'private', 'protected', 'virtual', 'namespace',
...
def obfuscate_identifiers(code):
    ...
    for id, obf_id in obfuscation_map.items():
        obfuscated_code = re.sub(r'\b' + id + r'\b', obf_id, obfuscated_code)  # Corrigido aqui
...
def obfuscate_literals(code):
    ...
    # Substituir strings
    code = re.sub(r'"[^"]*"', lambda x: '"' + ''.join(random.choice(string.printable) for _ in range(len(x.group()))) + '"', code)
    return code
